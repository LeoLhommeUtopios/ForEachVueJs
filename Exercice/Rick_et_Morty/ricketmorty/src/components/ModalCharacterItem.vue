      ...
      <!-- <a ref="characterModalRef" href="" @click.prevent="closeModal"> -->
        ...
        <div class="modal-container p-3 sm:p-5 fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 max-w-xs w-full sm:max-w-3xl flex flex-col sm:flex-row rounded-md border border-gray-200 bg-white drop-shadow-2xl">
          ...
          <div class="py-2 sm:py-0 sm:pl-8 sm:pr-28">
            ...
            <button v-if="!isFavorite(character.id)" v-on:click="addToFavorite(character.id)" type="button" class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow mb-2">add to fav</button>
            <button v-else v-on:click="removeToFavorite(character.id)" type="button" class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow mb-2">remove to fav</button>
          </div>
          
          <!-- Bouton de fermeture du modal -->
          <div v-on:click="closeModal" class="h-7 w-7 sm:h-6 sm:w-6 sm:text-xs rounded-full absolute top-5 right-5 flex justify-center items-center border border-gray-300 bg-gray-50 shrink-0">
